 ...
 * mcpinglib.c - see mcpinglib.h for design notes.
 */
...
int mcpinglib_buf_init(mcpinglib_buf_t *buf, size_t initial_cap) {
...
int mcpinglib_buf_reserve(mcpinglib_buf_t *buf, size_t additional) {
    ...
    /* FIX: Prevent integer overflow on buf->len + additional */
    if (additional > SIZE_MAX - buf->len) return -1;
...
int mcpinglib_write_varint(mcpinglib_buf_t *buf, int32_t value) {
    ...
    if (mcpinglib_buf_reserve(buf, (size_t)n) < 0) return -1;
...
int mcpinglib_write_string(mcpinglib_buf_t *buf, const char *s) {
    size_t slen = strlen(s);
    if (mcpinglib_write_varint(buf, (int32_t)slen) < 0) return -1;
    if (mcpinglib_buf_reserve(buf, slen) < 0) return -1;
...
int mcpinglib_write_u8(mcpinglib_buf_t *buf, uint8_t v) {
    if (mcpinglib_buf_reserve(buf, 1) < 0) return -1;
    buf->data[buf->len++] = v;
    return 0;
}

int mcpinglib_write_u16(mcpinglib_buf_t *buf, uint16_t v) {
    if (mcpinglib_buf_reserve(buf, 2) < 0) return -1;
    buf->data[buf->len++] = (unsigned char)((v >> 8) & 0xFF);
    buf->data[buf->len++] = (unsigned char)(v & 0xFF);
    return 0;
}

int mcpinglib_write_i32(mcpinglib_buf_t *buf, int32_t v) {
    if (mcpinglib_buf_reserve(buf, 4) < 0) return -1;
    uint32_t uv = (uint32_t)v;
    for (int i = 3; i >= 0; i--) buf->data[buf->len++] = (unsigned char)((uv >> (i * 8)) & 0xFF);
    return 0;
}

int mcpinglib_write_i64(mcpinglib_buf_t *buf, int64_t v) {
    if (mcpinglib_buf_reserve(buf, 8) < 0) return -1;
...
int mcpinglib_write_bool(mcpinglib_buf_t *buf, int v) {
    return mcpinglib_write_u8(buf, v ? 1 : 0);
}

int mcpinglib_write_bytes(mcpinglib_buf_t *buf, const void *data, size_t len) {
    if (mcpinglib_buf_reserve(buf, len) < 0) return -1;
...
int mcpinglib_write_uuid(mcpinglib_buf_t *buf, const unsigned char uuid[16]) {
    return mcpinglib_write_bytes(buf, uuid, 16);
}
...
int mcpinglib_read_string(mcpinglib_reader_t *r, char **out) {
    int32_t slen;
    if (mcpinglib_read_varint(r, &slen) < 0) return -1;
    if (slen < 0 || (size_t)slen > mcpinglib_reader_remaining(r)) return -1;
...
int mcpinglib_read_u8(mcpinglib_reader_t *r, uint8_t *out) {
    if (mcpinglib_reader_remaining(r) < 1) return -1;
    *out = r->data[r->pos++];
    return 0;
}

int mcpinglib_read_u16(mcpinglib_reader_t *r, uint16_t *out) {
    if (mcpinglib_reader_remaining(r) < 2) return -1;
...
int mcpinglib_read_i32(mcpinglib_reader_t *r, int32_t *out) {
    if (mcpinglib_reader_remaining(r) < 4) return -1;
    uint32_t v = 0;
...
int mcpinglib_read_i64(mcpinglib_reader_t *r, int64_t *out) {
    if (mcpinglib_reader_remaining(r) < 8) return -1;
    uint64_t v = 0;
...
int mcpinglib_read_bytes(mcpinglib_reader_t *r, void *out, size_t len) {
    if (mcpinglib_reader_remaining(r) < len) return -1;
...
int mcpinglib_connect(mcpinglib_conn_t *conn, const char *host, int port) {
    /* FIX: Initialize sock to prevent closing random FDs on early error */
    conn->sock = -1;
...
int mcpinglib_recv_packet(mcpinglib_conn_t *conn, int32_t *out_id,
    ...
    int32_t total_len;
    if (mcpinglib_sock_read_varint(conn->sock, &total_len) < 0) return -1;
    if (total_len < 0 || total_len > MCPINGLIB_MAX_PACKET_LEN) {
        errno = EMSGSIZE;
        return -1;
    }

    /* FIX: Prevent malloc(0) returning NULL and causing a segfault on empty packets */
    unsigned char *raw = malloc((size_t)total_len > 0 ? (size_t)total_len : 1);
    ...
    mcpinglib_reader_t r;
    mcpinglib_reader_init(&r, raw, (size_t)total_len);

    int32_t packet_id;
    if (mcpinglib_read_varint(&r, &packet_id) < 0) {
        free(raw);
        return -1;
    }

    size_t remaining = mcpinglib_reader_remaining(&r);
    unsigned char *payload = NULL;
    if (remaining > 0) {
        /* FIX: Prevent malloc(0) returning NULL here too for consistency */
        payload = malloc(remaining > 0 ? remaining : 1);
        if (payload == NULL) {
            free(raw);
            return -1;
        }
...
int mcpinglib_build_handshake(mcpinglib_buf_t *out, int32_t protocol_version,
    ...
    if (mcpinglib_write_varint(out, protocol_version) < 0) return -1;
    if (mcpinglib_write_string(out, host) < 0) return -1;
    if (mcpinglib_write_u16(out, port) < 0) return -1;
    if (mcpinglib_write_varint(out, next_state) < 0) return -1;
    return 0;
}
...
int mcpinglib_status_ping(const char *host, int port, int32_t protocol_version,
                    char **out_json) {
    mcpinglib_conn_t conn;
    if (mcpinglib_connect(&conn, host, port) < 0) return -1;

    mcpinglib_buf_t hs;
    if (mcpinglib_buf_init(&hs, 0) < 0 ||
        mcpinglib_build_handshake(&hs, protocol_version, host, (uint16_t)port, 1) < 0) {
        mcpinglib_buf_free(&hs);
        mcpinglib_close(&conn);
        return -1;
    }
    if (mcpinglib_send_packet(&conn, 0x00, &hs) < 0) {
        mcpinglib_buf_free(&hs);
        mcpinglib_close(&conn);
        return -1;
    }
    ...
    mcpinglib_buf_t empty;
    if (mcpinglib_buf_init(&empty, 1) < 0) {
        mcpinglib_close(&conn);
        return -1;
    }
    if (mcpinglib_send_packet(&conn, 0x00, &empty) < 0) {
        mcpinglib_buf_free(&empty);
        mcpinglib_close(&conn);
        return -1;
    }
    mcpinglib_buf_free(&empty);
    ...
    unsigned char *payload = NULL;
    if (mcpinglib_recv_packet(&conn, &resp_id, &payload, &payload_len) < 0) {
        mcpinglib_close(&conn);
        return -1;
    }
    mcpinglib_close(&conn);
    ...
    mcpinglib_reader_t r;
    mcpinglib_reader_init(&r, payload, (size_t)payload_len);
    char *json;
    if (mcpinglib_read_string(&r, &json) < 0) {
        free(payload);
        return -1;
    }
    free(payload);
